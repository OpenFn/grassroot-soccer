alterState(state => {
  ...
  function removeEmojis(string) {
    ...
    const result = string.replace(regex, '_');
    const val = `${result.substring(0, result.lastIndexOf('_'))}${result.substr(-10)}`;

    return val;
  }

  state.helperFunctions = { transform, removeEmojis };
...
upsert(
  ...
  fields(
    ...
    field('CommCare_Ext_ID__c', state => {
      return `${dataValue('form.case.@case_id')(state)}-${state.helperFunctions
        .removeEmojis(dataValue('form.hidden_properties.intervention_name')(state))
        .replace(/\//gi, '')}`;
    }),
